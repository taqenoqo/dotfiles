...
- 単純化を試した後は、振る舞いが変わっていないかを実測させる。判定は「差が小さいか」ではなく「差があるか」。「その入力は稀だから」で黙って適用させると、ユーザが選ぶ機会が消える。
- 空振りの回は一行で畳ませる。「検討した結果ちゃんと無かった」という説明も書かせない。毎編集後に走るので、空振りが長いと提案のある回が埋もれる。
- 「仕様を黙って狭めた」は一族として書く。直書きした名前やパスだけを例示すると、同じ一族の他の member (順に呼べば大丈夫、件数が少なければ大丈夫、この環境なら大丈夫) を拾えるかがモデルの一般化任せになる。実際、競合状態を見つける回と見つけない回が混ざった。
...
- 既存の仕組みとの重複検出は、当初モデル依存に見えた (Opus は「標準の `TZ` 環境変数が同じことをする」と気づき、Sonnet は同じ単位を「目的そのもの」として通した)。「自分で書かずに済ませる手段が既にないか確かめる」を §2 に追記した後は、Sonnet も `TZ` を実際に設定して挙動が同じことを確かめたうえで指摘するようになった。モデル差だと思われたものが本文で埋まった例。
...
- 評価ケースに見つかる欠陥が 2 つ以上あると、劇的な方 (データ消失など) に注意が集中し、もう一方は誰も報告しない。特定の指摘を測りたいケースは、その指摘以外に見つかるものが無い入力にすること。バックアップ関数のケース (競合状態 + タイムスタンプ精度) では 3 体全員が競合状態だけを報告した。欠陥を 1 つに絞ったケース (「中身が変わったか」を mtime/size で判定する実装) に作り直すと、3 体全員が同じ 1 点に収束し 7/7 で揃った。
- LLM に時刻依存の単純化を検証させると、時刻を固定してから測ることがある。マイクロ秒を足す案を、`datetime.now` を固定した状態でテストして「衝突は防げない」と結論した実例がある。単純化案が依存している当の仕組みを固定すると、必ず失敗する。
